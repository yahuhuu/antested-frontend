// Path: src/components/features/test-runs/details/CompletePlanConfirmationModal.tsx
import React from 'react';
import { StatusCounts } from '../../../../services/testRunService';
import ProgressBar from '../ProgressBar';

interface CompletePlanConfirmationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  statusCounts: StatusCounts;
  totalTestCases: number;
  hasNonCompletedRuns: boolean;
}
...
const CompletePlanConfirmationModal: React.FC<CompletePlanConfirmationModalProps> = ({ isOpen, onClose, onConfirm, statusCounts, totalTestCases, hasNonCompletedRuns }) => {
  if (!isOpen) {
    return null;
  }
  ...
  let warningMessage = 'Are you sure you want to mark this test plan as completed?';
  if (completionPercentage < 100) {
      warningMessage = `This test plan is only ${completionPercentage}% complete. ${warningMessage}`;
  } else if (hasNonCompletedRuns) {
      warningMessage = `Not all test runs within this plan are completed. ${warningMessage}`;
  } else if (failedTotal > 0 || statusCounts.blocked > 0) {
      warningMessage = `This test plan has failing or blocked test cases. ${warningMessage}`;
  }
      ...
      >
        <div className="p-6">
          ...
          <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg space-y-3">
              <h4 className="text-sm font-bold text-gray-700 dark:text-gray-200">Overall Progress</h4>
...
export default CompletePlanConfirmationModal;