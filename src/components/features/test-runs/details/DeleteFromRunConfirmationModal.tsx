// Path: src/components/features/test-runs/details/DeleteFromRunConfirmationModal.tsx
import React from 'react';

interface DeleteFromRunConfirmationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  itemCount: number;
...
const DeleteFromRunConfirmationModal: React.FC<DeleteFromRunConfirmationModalProps> = ({ isOpen, onClose, onConfirm, itemCount }) => {
  if (!isOpen) {
    return null;
  }
        ...
        <div className="p-6">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              Remove Test Cases from Run
            </h3>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
              Are you sure you want to remove {itemCount} test case{itemCount > 1 ? 's' : ''} from this test run?
              <br/><br/>
              This action will only remove them from this run's list and will not delete the test cases themselves.
            </p>
          </div>
...
export default DeleteFromRunConfirmationModal;